...
int buffer_size = 0;
int n_op_samples[1024] = {0};
int n_lost_op_samples[1024] = {0};
...
//#if 0
	int tmp = 0;
        ...
        tmp = read(fd, global_buffer[my_id], sizeof(ibs_op_t)/*buffer_size*/);
	//tmp = read(fd, global_buffer[my_id], sizeof(ibs_op_t));
        if (tmp <= 0) {
