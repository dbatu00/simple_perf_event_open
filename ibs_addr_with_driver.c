...
// before
	...
	for (cpu = 0; cpu < num_cpus; cpu++) {
		...
		ioctl(fds[cpu], SET_BUFFER_SIZE, buffer_size);
		//ioctl(fd[cpu], SET_POLL_SIZE, poll_size / sizeof(ibs_op_t));
		ioctl(fds[cpu], SET_MAX_CNT, op_cnt_max_to_set);
            	if (ioctl(fds[cpu], IBS_ENABLE)) {
