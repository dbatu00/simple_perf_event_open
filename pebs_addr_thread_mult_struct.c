...
//static char *our_mmap[NUM_THREADS];
static long sample_type;
static long read_format;
static int quiet;
//static long long prev_head[NUM_THREADS];
...
struct HashEntry {
   int tid;   
   int key;
} hashArray[NUM_THREADS];

int hashCode(int key) {
   return key % NUM_THREADS;
}
...
int get_tid(int key) {
   ...
   if(hashArray[hashIndex].key == key)
         return hashArray[hashIndex].tid;
	
   return -1;        
}

void insert_to_hash(int key,long tid) {
...
static void our_handler(int signum, siginfo_t *info, void *uc) {
	...
	int os_tid = syscall(SYS_gettid);

	long tid = get_tid(os_tid);
	printf("our_handler is called by thread %d\n", os_tid);

	ret=ioctl(fd, PERF_EVENT_IOC_DISABLE, 0);

	thread_data_array[tid].prev_head=perf_mmap_read(thread_data_array[tid].thread_mmap, MMAP_DATA_SIZE,thread_data_array[tid].prev_head,
...
void* thread_func(void* args) {
    long tid = (long) args;
    int os_tid = syscall(SYS_gettid);
    insert_to_hash(os_tid, tid);
    fprintf(stderr, "thread_func in thread %ld, os thread %d\n", tid, os_tid);
	...
	sample_type=PERF_SAMPLE_IP|PERF_SAMPLE_WEIGHT|PERF_SAMPLE_ADDR|PERF_SAMPLE_TID;
	read_format=0;
...
// insert here
	for(int i = 0; i < NUM_THREADS; i++) {
	   	thread_data_array[i].fd=perf_event_open(&pe, 0,-1,-1,0);
		if (thread_data_array[i].fd<0) {
			fprintf(stderr,"Error opening leader %llx\n", pe.config);
           	}
		//thread_data_array[i].thread_mmap=mmap(NULL, (1+MMAP_PAGES)*getpagesize(),PROT_READ|PROT_WRITE, MAP_SHARED, thread_data_array[i].fd, 0);
		thread_data_array[i].thread_mmap=mmap(NULL, mmap_pages*4096,PROT_READ|PROT_WRITE, MAP_SHARED, thread_data_array[i].fd, 0);
		//printf("fd: %d is for thread %d\n", thread_data_array[i].fd, i);
		fcntl(thread_data_array[i].fd, F_SETFL, O_RDWR|O_NONBLOCK|O_ASYNC);
	   	fcntl(thread_data_array[i].fd, F_SETSIG, SIGIO);

		struct f_owner_ex owner;
	   	owner.type = F_OWNER_TID;
	   	owner.pid  = hashArray[i].key;
		//printf("pid: %d\n", owner.pid);
           	fcntl(thread_data_array[i].fd, F_SETOWN_EX, &owner);
	}
	...
	our_mmap[0]=mmap(NULL, mmap_pages*4096,
	...
	struct f_owner_ex owner;
	owner.type = F_OWNER_TID;
	owner.pid  = hashArray[0].key;//syscall(SYS_gettid);
	printf("owner pid 0: %d\n", owner.pid);
        fcntl(fd, F_SETOWN_EX, &owner);

	our_mmap[1]=mmap(NULL, mmap_pages*4096,
        ...
        owner1.pid  = hashArray[1].key;//syscall(SYS_gettid);
	printf("owner pid 1: %d\n", owner1.pid);
        fcntl(fd1, F_SETOWN_EX, &owner1);*/

	//printf("this  point is reached\n");
	for(int i = 0; i < NUM_THREADS; i++) {
		   ioctl(thread_data_array[i].fd, PERF_EVENT_IOC_RESET, 0);
           	   ret=ioctl(thread_data_array[i].fd, PERF_EVENT_IOC_ENABLE,0);
		   if (ret<0) {
	...
	munmap(our_mmap[0],mmap_pages*4096);
	munmap(our_mmap[1],mmap_pages*4096);
	...
	for(int i = 0; i < NUM_THREADS; i++) {
                   /*ioctl(thread_data_array[i].fd, PERF_EVENT_IOC_RESET, 0);
                   ret=ioctl(thread_data_array[i].fd, PERF_EVENT_IOC_ENABLE,0);*/
		   ioctl(thread_data_array[i].fd, PERF_EVENT_IOC_DISABLE,0);
           	   //read(fd1,&bp_counter,sizeof(long long));
		   munmap(thread_data_array[i].thread_mmap,/*(1+MMAP_PAGES)*getpagesize()*/mmap_pages*4096);
           	   close(thread_data_array[i].fd);
        }
    }
    pthread_barrier_wait(&global_barrier);
    return NULL;
