...
static void our_handler(int signum,siginfo_t *oh, void *blah) {
	...
	if(status == RECEIVER) {
		...
		ret=ioctl(oh->si_fd, PERF_EVENT_IOC_DISABLE, 0);
        	pthread_mutex_lock(&lock);
        	sender_counter = sender_counter + 2;
        	while (sender_counter > overflow_count) {
                	printf("Waiting on condition variable cond1, sender_counter: %d, overflow_count: %d\n", sender_counter, overflow_count);
...
void* thread_func(void* args) {
    long tid = (long) args;
    int i;
    int bp_counter, read_result, bp_total_count, fd1, fd2, fd3;
